...
class ZettelChunkPayload(ChunkPayload):
    ...
    of text fragments. Unlike standard physical chunks, a Zettel encapsulates 
    a distinct, synthesized concept that possesses its own identity and 
    relationships within a knowledge graph.

    Attributes:
        id (uuid.UUID): A unique identifier for this specific Zettel, allowing it 
            to be referenced by other nodes in the graph. Defaults to a new UUID4.
        hypothesis (str): The synthesized atomic idea or central thesis of the 
            Zettel. This is the primary content used for semantic indexing and 
            subsequent annotations.
        tags (List[str]): A list of taxonomic keywords or categories associated 
            with this concept, used for grouping and filtering.
        related_zettel_ids (List[uuid.UUID]): A list of unique identifiers 
            pointing to other ZettelChunkPayloads generated within the same 
            execution context. This explicitly defines the edges of the 
            emergent knowledge graph.
        children (List[ChunkPayload]): Inherited from ChunkPayload. Contains 
            the original physical text fragments (evidence) that support the 
            hypothesis.
        annotations (Dict[str, Any]): Inherited from ChunkPayload. Stores 
            metadata generated by other strategies (e.g., EntityChunking) 
