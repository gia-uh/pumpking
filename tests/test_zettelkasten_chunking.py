...
class MockZettelProvider(ZettelProviderProtocol):
    def extract_zettels(
...
def test_zettelkasten_strategy_execution_flow():
    ...
    expected_zettel = ZettelChunkPayload(
        hypothesis="Hypothesis 1", children=[physical_chunk]
    )
    ...
    strategy = ZettelkastenChunking(
        splitter=mock_splitter, zettel_provider=mock_provider
    )
...
def test_zettelkasten_strategy_annotation_logic():
    ...
    zettel_result = ZettelChunkPayload(
        hypothesis="Apple Inc. released a new product.", children=[physical_chunk]
    )
